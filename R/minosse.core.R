...
minosse.core<-Vectorize(function(dtset,
  ...
  kriging_prediction<-function(x,new_mappa_ras,new_projection.ground,seed,configs){
    gimme.mod<-function(x,form,new_mappa_ras){
      form<-as.formula(paste("value~", paste(names(new_mappa_ras), collapse="+")));
      x[[1]]->x2
      ov<-sp::over(x[[1]],new_mappa_ras)
      x2@data<-cbind(x2@data,ov)
      x2[which(complete.cases(x2@data)),]->x2
      df_x2<-as.data.frame(x2)
      m<-minosse_fit.regModel(configs=configs,form, rmatrix=df_x2, new_mappa_ras,
      ...
      if(class(try(mod<-minosse_fit.gstatModel(configs=configs,x[[1]], form,new_mappa_ras,method=configs$kriging$method,vgmFun=as.character(resid_vg$var_model[2,1])),silent = TRUE))=="try-error") {
        mod<-NA} else  mod<-mod
      return(mod)
    }
    ...
    if(is.na(mod<-gimme.mod(x,form,new_mappa_ras))) {
      pred<-NA;
      kr_MINOSSE<-NA;
      kr_auc<-NA;
      data_for_th<-NA;
      ...
      MLA_mod<-NA} else {
        mod<-mod
        pred<-predict.gstatModel(mod,new_projection.ground,zmin=0,zmax=1,predict.method="RK")
        kr_MINOSSE<-raster(pred@predicted[,"value"])
        raster::crs(kr_MINOSSE)<-raster::crs(new_projection.ground)
        which(!is.na(raster::extract(kr_MINOSSE,x[[2]])))->valid_points
        if(class(try(kr_auc<-auc_wrapped(x[[2]][valid_points,]$value,raster::extract(kr_MINOSSE,x[[2]][valid_points,]))))=="try-error") kr_auc<-NA else kr_auc<-kr_auc
        RK_RMSE <- sqrt(mean((pred@validation$var1.pred-pred@validation$observed)^2))
        RK_tvar <- 1-var(pred@validation$residual, na.rm=T)/var(pred@validation$observed, na.rm=T)
        MLA_mod<-mod
        best_mod<-which(MLA_mod@regModel$results$RMSE==min(MLA_mod@regModel$results$RMSE))
        xgb_RMSE<-MLA_mod@regModel$results$RMSE[best_mod]
        xgb_R2<-MLA_mod@regModel$results$Rsquared[best_mod]
      }
